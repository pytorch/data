...
class S3Handler {
 ...
 public:
  ...
  void SetLastMarker(const Aws::String last_marker) {
    this->last_marker_ = last_marker;
  }
  void SetBufferSize(const uint64_t buffer_size) {
    this->buffer_size_ = buffer_size;
  }
  void SetMultiPartDownload(const bool multi_part_download) {
    this->use_multi_part_download_ = multi_part_download;
  }
  void ClearMarker();

  long GetRequestTimeoutMs() const {
    return s3_handler_cfg_->requestTimeoutMs;
  }
  Aws::String GetRegion() const {
    return s3_handler_cfg_->region;
  }
  Aws::String GetLastMarker() const {
    return last_marker_;
  }
  bool GetUseMultiPartDownload() const {
    return use_multi_part_download_;
  }
