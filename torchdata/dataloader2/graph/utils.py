...
def find_lca_non_shardable_dp(graph: DataPipeGraph) -> Optional[DataPipe]:
    r"""
    Given the graph of DataPipe generated by ``traverse_dps`` function, return the DataPipe
    instance that is the lowest common ancestor of all non-shardable DataPipes
    (``dp.is_shardable() == False``).

    Note:
      - If there are multiple branches sharding a same source DataPipe and
        any of branch contains non-shardable DataPipe, all branches are
        treated as non-shardable DataPipes.
      - If there is any non-shardable DataPipe in a circular-referenced (sub)graph,
        all DataPipes within this (sub)graph are treated as non-shardable DataPipes.
    """
    assert len(graph) == 1, "DataPipeGraph should only contain a single output DataPipe"

    def _is_non_shardable(dp: DataPipe) -> bool:
        return hasattr(dp, "is_shardable") and not dp.is_shardable()

    dps = list_dps(graph)
    non_shardable_dps = set()
    for dp in dps:
        if _is_non_shardable(dp):
            parent_dps = list_dps(traverse_dps(dp))
            for par_dp in parent_dps:
                non_shardable_dps.add(id(par_dp))
    ...
    def helper(root_dp_id, root_dp, root_graph) -> Optional[DataPipe]:  # pyre-ignore
        ...
        if len(non_shardable_parents) > 0:
            # One unique non-shardable DataPipe
            if len(non_shardable_parents) == 1 or all(dp == non_shardable_parents[0] for dp in non_shardable_parents):
                cache[root_dp_id] = non_shardable_parents[0]
