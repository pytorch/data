...
class TestTensorDataset(TestCase):
    ...
    def test_getitem(self):
        ...
        x = torch.randn(15, 10)
        source = TensorDataset(t, x)
        for i in range(15):
            self.assertEqual(t[i], source[i][0])
            self.assertEqual(x[i], source[i][1])
...
class TestStackDataset(TestCase):
    ...
    def test_getitem(self):
        t = TensorDataset(torch.randn(15, 10))
        x = TensorDataset(torch.randn(15, 5, 4))
        source = StackDataset(t, x)
        for i in range(15):
            self.assertEqual(t[i], source[i][0])
            self.assertEqual(x[i], source[i][1])
        source = StackDataset(a=t, b=x)
        for i in range(15):
            self.assertEqual(t[i], source[i]["a"])
            self.assertEqual(x[i], source[i]["b"])
    ...
    def test_getitems(self):
        ...
        for i in range(4):
            ...
            self.assertEqual(x[i], batch[i][1])

        source = StackDataset(t=t, x=x)
        batch = source.__getitems__([0, 1, 2, 3])
        for i in range(4):
            self.assertEqual(t[i], batch[i]["t"])
            self.assertEqual(x[i], batch[i]["x"])
...
def _test_timeout_pin_memory(persistent_workers):
    ...
    dataloader = DataLoader(
        dataset, batch_size=2, num_workers=2, timeout=1, pin_memory=True, persistent_workers=persistent_workers
    )
    _ = next(iter(dataloader))
...
def _test_large_sampler_indices(persistent_workers):
    ...
    dataloader = DataLoader(
        EmptyTensorDataset(10000000), batch_size=40960, persistent_workers=persistent_workers, num_workers=1
    )
...
def _test_get_worker_info():
    ...
    dataloader = DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, worker_init_fn=_test_worker_info_init_fn
    )
    it = iter(dataloader)
    data = []
    for d in it:
...
""",
    ...
    def test_bulk_loading_nobatch(self):
        ...
        for num_workers in [0, 4]:
            dl = self._get_data_loader(
                ds, num_workers=num_workers, batch_size=None, sampler=sampler, pin_memory=TEST_CUDA
            )
    ...
    def test_multiple_dataloaders(self):
        for multiprocessing_context in supported_multiprocessing_contexts:
            ...
            loader2_it = iter(
                self._get_data_loader(self.dataset, num_workers=2, multiprocessing_context=multiprocessing_context)
            )
            next(loader1_it)
            next(loader1_it)
            next(loader2_it)
    ...
    def test_invalid_ctor_args_combinations(self):
        ...
        with self.assertRaisesRegex(ValueError, "DataLoader with IterableDataset: expected unspecified batch_sampler"):
            self._get_data_loader(
                dataset,
                batch_sampler=torch.utils.data.BatchSampler(torch.utils.data.SequentialSampler(dataset), 3, False),
            )
        with self.assertRaisesRegex(ValueError, "DataLoader with IterableDataset: expected unspecified batch_sampler"):
    ...
    def test_iterable_style_dataset(self):
        ...
        for prefetch_factor in [2, 3, 4]:
            ...
            dataloader = self._get_data_loader(
                dataset, num_workers=num_workers, batch_size=7, prefetch_factor=prefetch_factor
            )
            dataloader_iter = iter(dataloader)
    ...
    def test_worker_seed_reproducibility(self):
        def get_dataloader():
            return DataLoader(
                dataset, batch_size=batch_size, num_workers=num_workers, generator=torch.Generator().manual_seed(42)
            )
    ...
    def test_distributed_sampler_invalid_rank(self):
        ...
        with self.assertRaisesRegex(ValueError, "Invalid rank"):
            DistributedSampler(dataset, 3, 3)
    ...
    def test_duplicating_data_with_drop_last(self):
        ...
        for i in range(num_processes):
            ...
            d_loader = self._get_data_loader(
                data_set, batch_size=int(num_batches / num_processes), drop_last=True, sampler=s
            )
    ...
    def test_sampler_reproducibility(self):
        ...
        for fn in (
            lambda: RandomSampler(
                self.dataset, num_samples=5, replacement=True, generator=torch.Generator().manual_seed(42)
            ),
            lambda: RandomSampler(self.dataset, replacement=False, generator=torch.Generator().manual_seed(42)),
            lambda: WeightedRandomSampler(
                weights, num_samples=5, replacement=True, generator=torch.Generator().manual_seed(42)
            ),
            lambda: WeightedRandomSampler(
                weights, num_samples=5, replacement=False, generator=torch.Generator().manual_seed(42)
            ),
        ...
        for is_iterable_dataset, use_workers, pin_memory, hold_iter_reference in itertools.product(
            ...
            for exit_method in exit_methods:
                ...
                try:
                    ...
                    if len(alive) > 0:
                        fail(
                            "worker process (pid(s) {}) did not terminate".format(", ".join(str(p.pid) for p in alive))
                        )
                    if exit_method is None:
                        ...
                        if exit_method == "loader_error":
                            if not isinstance(loader_p.exception, RuntimeError) or "Loader error" not in str(
                                loader_p.exception
                            ):
                            ...
                            if isinstance(loader_p.exception, RuntimeError):
                                if "DataLoader worker (pid" not in str(loader_p.exception):
                                    fail(
                                        "loader process did not raise expected exception, but had {}".format(
                                            loader_p.exception
                                        )
                                    )
                            ...
                            if not isinstance(loader_p.exception, RuntimeError) or "Worker error" not in str(
                                loader_p.exception
                            ):
                                fail(f"loader process did not raise expected exception, but had {loader_p.exception}")
                finally:
    ...
    def test_excessive_thread_creation_warning(self):
        with self.assertWarnsRegex(
            UserWarning, r"excessive worker creation might get DataLoader running slow or even freeze"
        ):
            DataLoader(self.dataset, batch_size=2, num_workers=1000)
    ...
    def test_sequential_batch(self):
        for persistent_workers in (False, True):
            if persistent_workers:
                loader = DataLoader(
                    self.dataset, batch_size=2, shuffle=False, persistent_workers=persistent_workers, num_workers=1
                )
            else:
...
@unittest.skipIf(IS_WINDOWS, "Needs fork")
@unittest.skipIf(
    TEST_WITH_ASAN, "This test hangs when running with ASAN, see https://github.com/pytorch/pytorch/issues/75492"
)
