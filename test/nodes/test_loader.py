...
def test_loader_correct_state_dict_at_midpoint(test, node: BaseNode, length: int):
    x = Loader(node)
    results = list(x)
    ...
    test.assertEqual(len(results), length)
    test.assertEqual(len(results_copy), length)
    test.assertEqual(results[length // 2 :], results_copy[length // 2 :])
...
class TestLoader(TestCase):
    def test_loader_equal_state_dict_on_save_load_iterable(self) -> None:
        ...
        test_loader_correct_state_dict_at_midpoint(self, node, length)
