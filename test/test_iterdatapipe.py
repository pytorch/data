...
def test_torchdata_pytorch_consistency() -> None:
    def extract_datapipe_names(module):
        return {
            ...
            and isinstance(dp_type, type)
            and issubclass(dp_type, IterDataPipe)
        }
...
class TestIterDataPipe(expecttest.TestCase):
    def test_in_memory_cache_holder_iterdatapipe(self) -> None:
        ...
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            cache_dp, n_elements_before_reset
        )
        ...
        with self.assertRaisesRegex(
            TypeError, "doesn't have valid length until the cache is loaded"
        ):
            len(cache_dp)
    ...
    def test_iter_key_zipper_iterdatapipe(self) -> None:
        ...
        self.assertEqual(
            [(i, f"{i},{i}") for i in range(10)], list(zip_dp_w_key_str_merge)
        )
        ...
        n_elements_before_reset = 5
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            zip_dp, n_elements_before_reset
        )
    ...
    def test_map_key_zipper_datapipe(self) -> None:
        ...
        it = iter(result_dp)
        with self.assertRaisesRegex(
            KeyError, "is not a valid key in the given MapDataPipe"
        ):
            next(it)
        ...
        result_dp = source_dp.zip_with_map(map_dp, odd_even)
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            result_dp, n_elements_before_reset
        )
    ...
    def test_cycler_iterdatapipe(self) -> None:
        ...
        n_elements_before_reset = 4
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            cycler_dp, n_elements_before_reset
        )
        ...
        with self.assertRaisesRegex(
            TypeError,
            "instance cycles forever, and therefore doesn't have valid length",
        ):
            len(cycler_dp)
    ...
    def test_header_iterdatapipe(self) -> None:
        ...
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            header_dp, n_elements_before_reset
        )
        ...
        with warnings.catch_warnings(record=True) as wa:
            ...
            self.assertRegex(
                str(wa[0].message),
                r"length of this HeaderIterDataPipe is inferred to be equal to its limit",
            )
    ...
    def test_enumerator_iterdatapipe(self) -> None:
        ...
        self.assertEqual(
            [(0, "a"), (1, "b"), (2, "c"), (3, "d"), (4, "e")], list(enum_dp)
        )

        # Functional Test: start index from non-zero
        enum_dp = source_dp.enumerate(starting_index=10)
        self.assertEqual(
            [(10, "a"), (11, "b"), (12, "c"), (13, "d"), (14, "e")], list(enum_dp)
        )

        # Reset Test:
        n_elements_before_reset = 2
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            enum_dp, n_elements_before_reset
        )
        self.assertEqual([(10, "a"), (11, "b")], res_before_reset)
        self.assertEqual(
            [(10, "a"), (11, "b"), (12, "c"), (13, "d"), (14, "e")], res_after_reset
        )
    ...
    def test_index_adder_iterdatapipe(self) -> None:
        ...
        n_elements_before_reset = 2
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            index_adder_dp, n_elements_before_reset
        )
    ...
    def test_line_reader_iterdatapipe(self) -> None:
        ...
        source_dp = IterableWrapper(
            [("file1", io.StringIO(text1)), ("file2", io.StringIO(text2))]
        )
        ...
        source_dp = IterableWrapper(
            [
                ("file1", io.BytesIO(text1.encode("utf-8"))),
                ("file2", io.BytesIO(text2.encode("utf-8"))),
            ]
        )
        line_reader_dp = source_dp.readlines()
        expected_result_bytes = [
            ("file1", line.encode("utf-8")) for line in text1.splitlines()
        ...
        n_elements_before_reset = 2
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            line_reader_dp, n_elements_before_reset
        )
    ...
    def test_paragraph_aggregator_iterdatapipe(self) -> None:
        ...
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            para_agg_dp, n_elements_before_reset
        )
    ...
    def test_rows_to_columnar_iterdatapipe(self) -> None:
        ...
        source_dp = IterableWrapper(
            [
                [
                    {l: i for i, l in enumerate("abc")},
                    {l: i * 10 for i, l in enumerate("abc")},
                ],
                [
                    {l: i + 100 for i, l in enumerate("abc")},
                    {l: (i + 100) * 10 for i, l in enumerate("abc")},
                ],
            ]
        )
        ...
        batch2 = defaultdict(
            list, {"a": [100, 1000], "b": [101, 1010], "c": [102, 1020]}
        )
        ...
        source_dp = IterableWrapper(
            [
                [
                    [i for i, _ in enumerate("abc")],
                    [i * 10 for i, _ in enumerate("abc")],
                ],
                [
                    [i + 100 for i, _ in enumerate("abc")],
                    [(i + 100) * 10 for i, _ in enumerate("abc")],
                ],
            ]
        )
    ...
    def test_sample_multiplexer_iterdatapipe(self) -> None:
        ...
        with self.assertRaisesRegex(
            ValueError, "Expecting a positive and non-zero weight"
        ):
        ...
        n_elements_before_reset = 5
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            sample_mul_dp, n_elements_before_reset
        )
    ...
    def test_in_batch_shuffler_iterdatapipe(self) -> None:
        ...
        n_elements_before_reset = 2
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            filtered_dp, n_elements_before_reset
        )
    ...
    def test_max_token_bucketizer_iterdatapipe(self) -> None:
        ...
        with self.assertRaises(
            ValueError,
            msg="``max_token_count`` must be equal to or greater than ``max_len``.",
        ):
            source_dp.max_token_bucketize(max_token_count=2, max_len=3)

        # Functional Test: Filter out min_len
        batch_dp = source_dp.max_token_bucketize(
            max_token_count=5, min_len=2, buffer_size=10
        )
        exp_batch = [
            ["11", "22"],
            ["111"],
        ...
        batch_dp = source_dp.max_token_bucketize(
            max_token_count=5, max_len=4, buffer_size=10
        )
        ...
        batch_dp = source_dp.max_token_bucketize(
            max_token_count=7, len_fn=_custom_len_fn, buffer_size=10
        )
        exp_batch = [
            ["1", "2", "11"],
            ["22", "111"],
    ...
    def test_map_batches_iterdatapipe(self):
        ...
        n_elements_before_reset = 5
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            batch_mapped_dp, n_elements_before_reset
        )
        ...
        batch_mapped_input_2_dp = source_dp.map_batches(
            fn_2_cols, batch_size=9, input_col=[1, 2]
        )
        ...
        with self.assertRaisesRegex(
            TypeError, "length relies on the output of its function."
        ):
            len(batch_mapped_dp)
    ...
    def test_flatmap_iterdatapipe(self):
        ...
        self.assertEqual(
            list(itertools.chain(*[(-2, 2) for _ in range(20)])), list(input_col_2_dp)
        )

        # Reset Test: reset the DataPipe after reading part of it
        n_elements_before_reset = 5
        res_before_reset, res_after_reset = reset_after_n_next_calls(
            flatmapped_dp, n_elements_before_reset
        )
    ...
    def test_unzipper_iterdatapipe(self):
        ...
        (dp2,) = source_dp.unzip(
            sequence_length=3, columns_to_skip=[0, 2], buffer_size=0
        )
        self.assertEqual(list(range(10, 20)), list(dp2))
    ...
    def test_itertomap_mapdatapipe(self):
        ...
        map_dp = source_dp.to_map_datapipe()
        with self.assertRaisesRegex(
            TypeError, "Cannot convert dictionary update element"
        ):
            _ = list(map_dp)
        ...
        self.assertEqual(
            list(range(1, 11)), [map_dp["k" + str(idx)] for idx in range(10)]
        )
        self.assertFalse(map_dp._map is None)
