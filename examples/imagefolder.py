...
from torch.utils.data.datapipes.iter import HttpReader, ListDirFiles

IMAGES_ROOT = 'imagefolder_images'
# IMAGES_ROOT = '/home/vitaly/dataset/images_dataset/train'
...
def test_speed(dataloader, test_name=''):
    c = 0
    t = time.time()
    warmup = True
    ...
    for image, label in dataloader:
        if warmup:
            print(test_name, 'warmup took', time.time() - t, 'seconds')
            warmup = False
        c += BATCH_SIZE
        if c % 50000 == 0:
            print(test_name, c, 'images loaded', (time.time() - t) /
                  c * 1000, 'sec per 1000 images')
    print(test_name, c, 'images loaded', (time.time() - t) /
          c * 1000, 'sec per 1000 images')


# test_speed(dl, 'ImageFolder dataset implmentation')
...
def MyImageFolder(root=IMAGES_ROOT, transform=None):
    if not USE_FORK_DATAPIPE:
        ...
        # too large
        list_files_0 = ListDirFiles(root=IMAGES_ROOT, recursive=True)
        list_files_1 = ListDirFiles(
...
# test_speed(dl, 'DataPipes implementation')
...
HTTP_PATH_CAT = 'http://localhost:8000/cat/{1..99}.jpg'
HTTP_PATH_DOG = 'http://localhost:8000/dog/{1..99}.jpg'
...
test_speed(dl, 'DataPipes HTTP implementation')
