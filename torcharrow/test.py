...
from torcharrow import (
    Column, _NumericalColumn, _ListColumn, _StringColumn, _MapColumn, _BooleanColumn, _StructColumn)
# dataframe
...
class TestFlatColumn(unittest.TestCase):
    ...
    def test_flat(self):
        ...
        self.assertEqual(empty_i64_column._count, 0)
        ...
        self.assertEqual(coln._count, 4)
...
class TestNestedColumn(unittest.TestCase):
    ...
    def test_empty(self):
        ...
        self.assertTrue(isinstance(empty_i64_column._item_data, _NumericalColumn))
        self.assertEqual(empty_i64_column._item_data._dtype, int64)
    ...
    def test_nested_numerical_once(self):
        ...
        self.assertEqual(col._item_data._offset, 0) 
        for i in range(4):
            self.assertEqual(col._offsets[i], len(col._item_data._data) )
            col.append(list(range(i)))
            self.assertEqual(list(col[i]), list(range(i)))
            self.assertEqual(col._validity[i], True)
            self.assertEqual(col._offsets[i+1], len(col._item_data._data))

        verdict = [list(range(i)) for i in range(4)]

        for i,lst in zip(range(4), verdict):
            self.assertEqual(col[i], lst)

        col.extend([[-1,-2,-3],[-4,-5]])
        # this is the last validity...
        self.assertEqual(col._item_data._validity[col._offsets[-1]-1], True)
