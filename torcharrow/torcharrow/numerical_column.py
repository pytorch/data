...
from torcharrow.dtypes import  is_boolean, is_numerical, NL
from torcharrow.column import AbstractColumn, _set_column_constructor
...
class NumericalColumn(AbstractColumn):
    ...
    def append(self, x):
        ...
        if x is None:
            ...
            self._data.append(x) # throws TypeError (for any non expected type)
    ...
    def __repr__(self):
        tab = tabulate([  [l if l is not None else 'None'] for l in self ],tablefmt='plain',showindex=True)
...
class _NumericalRepr:
    ...
    def __repr__(self):
        me = self.parent
        tab = tabulate([[l if l is not None else 'None',v] for (l,v) in zip(me._data, me._validity)],['data', 'validity'])
...
class BooleanColumn(NumericalColumn):
    ...
    def __repr__(self):
        tab = tabulate([  [str(bool(l)) if l is not None else 'None'] for l in self ],tablefmt='plain',showindex=True)
        typ = f"dtype: {self._dtype}, count: {self._length}, null_count: {self._null_count}" 
        return tab+NL+typ
...
class _BooleanRepr:
    ...
    def __repr__(self):
        me = self.parent
        tab = tabulate([[str(bool(l)) if l is not None else 'None',v] for (l,v) in zip(me._data, me._validity)],['data', 'validity'])
        typ = f"dtype: {me._dtype}, count: {me._length}, null_count: {me._null_count}, offset: {me._offset}"
        return tab+NL+typ
