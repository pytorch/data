...
# operator should move elseweher..
import operator
        ...
        if self.nullable:
            for f in self.fields:
                if not f.dtype.nullable:
                    raise TypeError(f'nullable structs require each field (like {f.name}) to be nullable as well.')
...
def derive_dtype(left_dtype, op):
    ...
    raise AssertionError(f"derive_dtype, unexpected type {left_dtype} for operation {op}")
