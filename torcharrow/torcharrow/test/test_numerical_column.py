...
from torcharrow import (Boolean, BooleanColumn, Column, Float64, Int64,
                        NumericalColumn, String, boolean, float64, int64,
                        is_numerical, string)
...
class TestNumericalColumn(unittest.TestCase):
    ...
    def test_map_where_filter(self):
        ...
        # maps None
        self.assertEqual(list(col.map({None: 1, 3: 33}, na_action='ignore')), [
                         1, 1, 1, 33, None, None])

        # keep None
        self.assertEqual(list(col.map({None: 1, 3: 33}, na_action=None)), [
                         None, None, None, 33, None, None])

        # maps as function
        self.assertEqual(list(col.map(lambda x: 1 if x is None else 33 if x ==
                         3 else x, na_action='ignore')), [1, 1, 1, 33, 4, 5])

        # where
        self.assertEqual(list(col.where(
            [True, False]*3, [i*i for i in range(6)])), [None, 1, None, 9, 4, 25])
    ...
    def test_sort_stuff(self):
        ...
        self.assertEqual(list(Column([None, 1, 5, 2]).sort_values(
            na_position='first')), [None, 1, 2, 5])
    ...
    def test_agg_handling(self):
        ...
        self.assertEqual(list(C.cummin()),  [min(
            c[:i]) for i in range(1, len(c)+1)]+[None])
        self.assertEqual(list(C.cummax()),  [max(
            c[:i]) for i in range(1, len(c)+1)]+[None])
        self.assertEqual(list(C.cumsum()),  [sum(
            c[:i]) for i in range(1, len(c)+1)]+[None])
