...
from torcharrow.dtypes import int64, Int64, string,  List_
...
class TestListColumn(unittest.TestCase):
    ...
    def test_map_reduce_etc(self):
        c= Column(List_(string))
        c.extend([ ['The', 'fox'], ['jumps'], ['over', 'the', 'river']])
        self.assertEqual(list(c.list.map(str.upper)), [['THE', 'FOX'], ['JUMPS'], ['OVER', 'THE', 'RIVER']])
        self.assertEqual(list(c.list.filter(lambda x: x.endswith('fox'))), [['fox'], [], []])
