...
from torcharrow import Session, ListColumn, NumericalColumn,  List_,  Int64, int64, string
...
class TestListColumn(unittest.TestCase):
    ...
    def test_empty(self):
        c = self.ts.Column(List_(int64))

        self.assertTrue(isinstance(c, ListColumn))
        self.assertEqual(c.dtype, List_(int64))

        self.assertTrue(isinstance(c._data, NumericalColumn))
        self.assertEqual(c._data.dtype, int64)
    ...
    def test_nested_numerical_twice(self):
        c = self.ts.Column(List_(List_(Int64(nullable=False), nullable=True), nullable=False))
        vals = [[[1, 2], None, [3, 4]], [[4], [5]]]
        c = c.append(vals)
        self.assertEqual(vals, list(c))
        
        d = self.ts.Column(List_(List_(Int64(nullable=False), nullable=True), nullable=False))
        for val in vals:
            d = d.append([val])
        self.assertEqual(vals, list(d))

    def test_nested_string_once(self):
        c = self.ts.Column(List_(string))
        c=c.append([[]])
        c=c.append([["a"]])
        c=c.append([["b", "c"]])
    ...
    def test_nested_string_twice(self):
        c = self.ts.Column(List_(List_(string)))
        c = c.append([[]])
        c = c.append([[[]]])
        c = c.append([[["a"]]])
    ...
    #     self.ts.Column([1, 2, 3, 4]).map(str, dtype=string)
