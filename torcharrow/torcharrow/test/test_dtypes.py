...
# schemas and dtypes
from torcharrow import (Boolean, Field, Float64, Int64, List_, Map, Schema,
                        String, Struct, boolean, float64, int64, is_numerical,
                        string)
...
class TestTypes(unittest.TestCase):
    ...
    def test_list(self):
        self.assertEqual(str(List_(Int64(nullable=True))),
                         "List_(Int64(nullable=True))")
        self.assertEqual(
    ...
    def test_struct(self):
        self.assertEqual(str(Struct([Field('a', int64), Field(
            'b', string)])), "Struct([Field('a', int64), Field('b', string)])")
        self.assertEqual(
            Struct([Field('a', int64), Field('b', string)]).typecode, "+s")
