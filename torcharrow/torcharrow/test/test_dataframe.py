...
from torcharrow import numerical_column
...
# schemas and dtypes
from torcharrow.dtypes import (Struct, Field, int64, int32, float64, Boolean, Int64, string, Float64, boolean, String, is_numerical)
...
class TestNumericalColumn(unittest.TestCase):
    ...
    def test_internals_full_nullable(self):
        with self.assertRaises(TypeError):
            ...
            df= DataFrame(Struct([Field('a',int64),Field('b',int64)], nullable=True))
        df = DataFrame(Struct([Field('a',int64.with_null()),Field('b',Int64(True))], nullable=True))
    ...
    def test_metastuff(self):
        df= DataFrame(Struct([Field('a',Int64(True)),Field('b',Int64(True))], nullable=True))
        df.extend([None]*3)
    ...
    def test_infer(self):
        ...
        self.assertEqual(df.dtype, Struct([Field('a',int64), Field('b',Float64(nullable=True))]))
    ...
    def test_sort_stuff(self):
        df = DataFrame({'a':[1,2,3], 'b': [1.0,None,3]})
        self.assertEqual(list(df.sort_values(by='a', ascending=False)),list(zip([3,2,1], [3,None,1.0])))
        # Not allowing None in comparison might be too harsh... 
        with self.assertRaises(TypeError):
            #TypeError: '<' not supported between instances of 'NoneType' and 'float'
            self.assertEqual(list(df.sort_values(by='b', ascending=False)),list(zip([3,2,1], [3,None,1.0])))


        df = DataFrame({'a':[1,2,3], 'b': [1.0,None,3], 'c': [4,4,1]})
        self.assertEqual(list(df.sort_values(by=['c','a'], ascending=False)),list([(2, None, 4), (1, 1.0, 4), (3, 3.0, 1)]))
             
        self.assertEqual(list(df.nlargest(n=2, columns=['c','a'],keep='first')),[(2, None, 4), (1, 1.0, 4)])
        self.assertEqual(list(df.nsmallest(n=2, columns=['c','a'], keep='first')), [(3, 3.0, 1), (1, 1.0, 4)])
    ...
    def test_operators(self):
        ...
        with self.assertRaises(TypeError): #TypeError: integer argument expected, got float
        ...
        self.assertEqual(list(c.radd(f,fill_value =100)),  [(i,) for i in[100, 2, 6, 200]])
    ...
    def test_agg_handling(self):
        ...
        self.assertEqual(list(C.cummin()),   [(i,) for i in[min(c[:i]) for i in range(1,len(c)+1)]+[None]])
        self.assertEqual(list(C.cummax()),   [(i,) for i in[max(c[:i]) for i in range(1,len(c)+1)]+[None]])
    ...
    def test_drop_keep_rename_reorder(self):
        ...
        self.assertEqual(list(df.rename({'a':'c', 'c':'a'})), [(1, 111), (2, 222), (3, 333)])
