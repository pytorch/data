...
class TestStringColumn(unittest.TestCase):
    ...
    def test_string_lifted_methods(self):
        ...
        self.assertEqual(list(
            Column(['1', '', '+3.0', '-4']).str.isinteger()), [True, False, False, True])
        self.assertEqual(list(
            Column(['1.0', '', '+3.e12', '-4.0']).str.isfloat()), [True, False, True, True])
        ...
        self.assertEqual(list(Column(['.abc', 'abc.a', '_']).str.isidentifier()), [
                         False, False, True])
        ...
        self.assertEqual(list(Column(['A B C', 'abc', ' ']).str.istitle()), [
                         True, False, False])
        self.assertEqual(
            list(Column(['UPPER', 'lower']).str.isupper()), [True, False])

        self.assertEqual(list(Column(['UPPER', 'lower']).str.capitalize()),  [
                         'Upper', 'Lower'])
        self.assertEqual(list(Column(['UPPER', 'lower']).str.swapcase()),  [
                         'upper', 'LOWER'])
        self.assertEqual(list(Column(['UPPER', 'lower']).str.lower()),  [
                         'upper', 'lower'])
        self.assertEqual(list(Column(['UPPER', 'lower']).str.upper()),  [
                         'UPPER', 'LOWER'])
        self.assertEqual(list(Column(['UPPER', 'lower', 'midWife']).str.casefold()), [
                         'upper', 'lower', 'midwife'])
        # Todo
        # self.assertEqual(list(Column(['1', '22', '33']).str.repeat(2)), [])
        self.assertEqual(list(Column(['UPPER', 'lower', 'midWife']).str.pad(
            width=10, side="center", fillchar="_")), ['__UPPER___', '__lower___', '_midWife__'])
        # ljust, rjust, center
        self.assertEqual(
            list(Column(['1', '22']).str.zfill(3)), ['001', '022'])
        self.assertEqual(list(Column(s).str.translate({ord('.'): ord('_')})), [
                         'hello_this', 'is_interesting_', 'this_is_24', 'paradise'])

        self.assertEqual(list(Column(s).str.count('.')), [1, 2, 1, 0])
        self.assertEqual(list(Column(s).str.startswith('h')),
                         [True, False, False, False])
        self.assertEqual(list(Column(s).str.endswith('this')),
                         [True, False, False, False])
