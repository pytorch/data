...
from torcharrow import (Boolean, Column, DataFrame, DType, Float32, Float64,
                        ...
                        from_arrow_table, from_arrow_array, from_pandas_dataframe, from_pandas_series)
...
class TestInterop(unittest.TestCase):
    ...
    def test_panda_series(self):
        ...
        self.assertEqual(list(_column_without_nan(s, Float64(True))),
                         list(from_pandas_series(s)))

        s = pd.Series([1, 2, 3])
        self.assertEqual(list(s),
                         list(from_pandas_series(s, Int16(False))))
    ...
    def test_panda_dataframes(self):
        ...
        self.assertEqual([(i,) for i in s['a']],
                         list(from_pandas_dataframe(s)))

        s = pd.DataFrame({'a': [1.0, np.nan, 3]})
        t = from_pandas_dataframe(s)
        self.assertEqual([(i,) for i in list(_column_without_nan(s['a'], Float64(True)))],
                         list(t))
        ...
        t = from_pandas_dataframe(s)
        self.assertEqual(t, DataFrame(
            {'a': Column(['a']), 'b': Column([1]), 'c': Column([True]), 'd': Column([1.0])}))
    ...
    def test_arrow_array(self):
        ...
        self.assertEqual([i.as_py() for i in s],
                         list(from_arrow_array(s, Int16(False))))

        s = pa.array([1, 2, 3])
        t = from_arrow_array(s)
        self.assertEqual(t.dtype, Int64(False))
        self.assertEqual([i.as_py() for i in s],
                         list(from_arrow_array(s)))
    ...
    def test_arrow_table(self):
        ...
        t = from_arrow_table(s)
        self.assertEqual(t.dtype, Struct([Field('a', Int64(False))]))
        self.assertEqual([(i.as_py(),) for i in s['a']],
                         list(t))
